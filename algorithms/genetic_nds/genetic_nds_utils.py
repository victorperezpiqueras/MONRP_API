import random
from algorithms.models.population import Population
...
class GeneticNDSUtils:
    ...
    # SELECTION------------------------------------------------------------------
    def selection_tournament2(self, population):
        ...
        for i in range(0, len(population)):
            ...
            for j in range(0, self.selection_candidates):
                ...
                if(candidate.total_score > best_total_score):
                    best_total_score = candidate.total_score
    ...
    def selection_tournament(self, population):
        ...
        for i in range(0, len(population)):
            ...
            for j in range(0, self.selection_candidates):
                ...
                # guardar el candidato que mejor score tiene
                if (best_candidate is None or candidate.total_score > best_score):#best_candidate.total_score
                    #best_candidate = copy.deepcopy(candidate)
            ...
            new_population.append(
                copy.deepcopy(population.get(best_index))
                )
        # retornar nueva poblacion
        return new_population
    ...
    def replacement_elitism(self, population, newpopulation):
        ...
        worst_individual_total_score = 10 ** 9
    ...
    def calculate_spread(self, population):
        ...
        df = self.eudis2([first_solution.objectives[0].value,
                          first_solution.objectives[1].value], first_extreme)
        dl = self.eudis2([last_solution.objectives[0].value,
                          last_solution.objectives[1].value], last_extreme)
